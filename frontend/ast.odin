...
Node_File_Flags :: bit_set[Node_File_Flag]
...
Node_Package :: struct {
    ...
    name: string,
    fullpath: string,
    files: map[string]^Node_File, // Note(Dragos): Need to change this eventually

    user_data: rawptr,
}

Node_File :: struct { // Todo(Dragos): Learn what's the difference between this and the C++ alternative
    using node: Node,
    id: int,
    pkg: ^Node_Package,
    ...
    docs: ^Node_Comment_Group,

    pkg_decl: ^Node_Package_Decl,
    pkg_token: Token,
    pkg_name: string,

    decls: [dynamic]^Stmt_Base,
    imports: [dynamic]^Node_Import_Decl,
    directive_count: int,

    comments: [dynamic]^Node_Comment_Group,
...
// Base Types
Expr_Base :: struct {
    using base_expr: Node,
    derived_expr: Any_Expr,
}

Stmt_Base :: struct {
    using base_stmt: Node,
...
// Expressions
Expr_Bad :: struct {
    using node: Expr_Base,
}

Expr_Ident :: struct {
    using node: Expr_Base,
    name: string, // Todo(Dragos): Change this to work with type checker
}

Expr_Implicit :: struct {
    using node: Expr_Base,
    token: Tokenizer,
}
...
Expr_Undef :: struct {
    using node: Expr_Base,
    token_kind: Token_Kind, // Learn(Dragos): Should this be a Token?
}

Expr_Basic_Lit :: struct {
    using node: Expr_Base,
    token: Token,
}
...
Expr_Proc_Type :: struct {
    ...
    inlining: Proc_Inlining,
    where_token: Token,
    where_clauses: []^Expr_Base,
}

Expr_Comp_Lit :: struct {
    using node: Expr_Base,
    type: ^Expr_Base,
    open: Token_Pos,
    elems: []^Expr_Base,
...
Expr_Binary :: struct {
    using node: Expr_Base,
    left, right: ^Expr_Base,
    op: Token,
}

Expr_Paren :: struct {
    using node: Expr_Base,
    open, close: Token_Pos,
    expr: ^Expr_Base,
}
...
Expr_Implicit_Selector :: struct {
    using node: Expr_Base,
    field: ^Expr_Ident,
}

Expr_Selector_Call :: struct {
    using node: Expr_Base,
    expr: ^Expr_Base,
    call: ^Expr_Call,
    modified_call: bool,
...
Expr_Matrix_Index :: struct {
    using node: Expr_Base,
    expr: ^Expr_Base,
    open, close: Token_Pos,
    row_index, column_index: ^Expr_Base,
}
...
Expr_Or_Return :: struct {
    using node: Expr_Base,
    expr: ^Expr_Base,
    token: Token,
}

Expr_Type_Assertion :: struct {
    using node: Expr_Base,
    expr: ^Expr_Base,
    dot: Token_Pos,
    open, close: Token_Pos,
...
Expr_Auto_Cast :: struct {
    using node: Expr_Base,
    op: Token,
    expr: ^Expr_Base,
}

Expr_Inline_Asm :: struct {
    using node: Expr_Base,
    token: Token,
    param_types: []^Expr_Base,
    return_types: ^Expr_Base,
    has_side_effects, is_align_stack: bool,
    dialect: Inline_Asm_Dialect_Kind,
    open, close: Token_Pos,
    constraints_string, asm_string: ^Expr_Base,
}
...
Stmt_Inline_Range :: struct {
    using node: Stmt_Base,
    label: ^Expr_Base,
    inline_pos: Token_Pos,
    for_pos: Token_Pos,
    val0, val1: ^Expr_Base,
    in_pos: Token_Pos,
    expr: ^Expr_Base,
...
Stmt_Case_Clause :: struct {
    using node: Stmt_Base,
    case_pos: Token_Pos,
    list: []^Expr_Base,
    terminator: Token_Pos,
    body: []^Stmt_Base,
}
...
// Note(Dragos): Do they really need to be pointers?
Any_Node :: union {
